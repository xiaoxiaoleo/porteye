...
# 数据库连接 & 全局扫描参数
global_dbcoon = 'mysql+mysqldb://evileo:5czVbOo4DOok45SvPHg9@sniffer.huobidev.com:3306/sslcloud'
...
def do_nmap_scan(targets,port):
	...
	while True:
		...
		try:
			nmap_proc = NmapProcess(targets=targets, options=options, safe_mode=False,fqp = '/usr/bin/nmap')
...
def run_wyportmap(targets, port,taskid=None):
	print '-' * 50
	print '* Starting id:(%s) [%s] portmap scan' % (taskid, targets)
	print '-' * 50
	nmap_result = do_nmap_scan(targets,port)
	print '-' * 50
...
if __name__ == "__main__":
	if len(sys.argv) == 3:
		...
		print run_wyportmap(sys.argv[1], sys.argv[2], sys.argv[3])
	else:
		print ("usage: %s targets ports taskid" % sys.argv[0])
		sys.exit(-1)
